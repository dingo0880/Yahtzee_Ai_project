...
# --- AI 핵심 두뇌: 몬테카를로 시뮬레이션 (결함 수정됨) ---
def estimate_expected_score(dice, keep_idxs, scoreboard, turn, rolls_left, n_sim=500):
    """
    [v1.5의 핵심] keep_idxs를 고정하고, '실제 남은 횟수(rolls_left)'만큼
    재굴림했을 때의 기대 점수를 Monte Carlo 방식으로 계산합니다.
    """
    total = 0
    ...
    for _ in range(n_sim):
        ...
        best_cat = max(
            ...
            key=lambda c: score_category(sim_dice, c) * w.get(c, 1.0),
            default="Chance"
        )
...
def strategic_keep_elite(dice, scoreboard, turn, rolls_left):
    ...
    for keep_idxs in unique_cands:
        ev = estimate_expected_score(dice, keep_idxs, scoreboard, turn, rolls_left, n_sim=200)
        if ev > best_ev:
...
def strategic_decide_dice_to_keep(dice, scoreboard, turn, cpu_type, rolls_left=2):
    ...
    if cpu_type == "공격형":
        return strategic_keep_attack(dice, scoreboard, turn)
    if cpu_type == "안정형":
        return strategic_keep_defense(dice, scoreboard, turn)
...
def cpu_select_category(dice, scoreboard, cpu_type, turn):
    ...
    max_scores = {
        ...
        "Four of a Kind": 30, "Full House": 25, "Small Straight": 15, "Large Straight": 30,
        "Yahtzee": 50, "Chance": 30
    }
    regrets = [(c, max_scores.get(c, 30) - score_category(dice, c)) for c in possible]
