...
def estimate_expected_score(dice, keep_idxs, scoreboard, turn, rolls_left, cpu_type, n_sim):
    ...
    for _ in range(n_sim):
        ...
        # 시뮬레이션 내부에서도 각 CPU 유형에 맞는 선택 로직 사용
...
def strategic_keep_elite(dice, scoreboard, turn, rolls_left):
    ...
    for keep_idxs in unique_cands:
        ev = estimate_expected_score(dice, keep_idxs, scoreboard, turn, rolls_left, "엘리트형", n_sim=1000)
        if ev > best_ev:
...
def strategic_keep_gambler(dice, scoreboard, turn, rolls_left):
    ...
    best_keep, best_ev = [], -1
    for keep_idxs in unique_cands:
        # 도박형은 더 많은 계산량으로 단순 전략을 보완
        ev = estimate_expected_score(dice, keep_idxs, scoreboard, turn, rolls_left, "도박형", n_sim=1000)
        if ev > best_ev:
            best_ev, best_keep = ev, keep_idxs
    return best_keep
...
def get_candidate_keeps(dice, scoreboard, turn):
    ...
    # 추천 목표는 각 AI의 dynamic_weights를 따름 (여기서는 단순화하여 공통 로직 사용)
    w = dynamic_weights_gambler(turn, scoreboard) # 단순한 추천 목표 사용
    possible = [c for c, s in scoreboard.items() if s is None]
    if possible:
